 ...
 */
GetOpt::GetOpt() {
	// XXX: disabled to get rid of QtGui dependency.
	qFatal("GetOpt(offset): disabled to get rid of QtGui dependency");
/*	if(!qApp)
		qFatal(
				"GetOpt: requires a QApplication instance to be constructed first");
...
void GetOpt::init(int argc, char *argv[], int offset) {
	current_required_arg = current_optional_arg = 0;
...
bool GetOpt::parse(bool untilFirstSwitchOnly) {
	...
	for(QStringList::const_iterator it = args.end(); it != args.begin() ;)
		stack.push(*--it);

	const OptionConstIterator obegin = options.begin();
	const OptionConstIterator oend = options.end();
	enum {
		StartState, ExpectingState, OptionalState
	} state = StartState;
	Option currOpt;
	enum TokenType {
		LongOpt, ShortOpt, Arg, End
	} t, currType = End;
	...
	while (!stack.isEmpty() || extraLoop) {
		...
		case StartState:
			if(opt.type == OSwitch) {
				...
				if(current_required_arg < required_positional_args.size())
					*required_positional_args[current_required_arg++].string_value = a;
				else if(current_optional_arg < optional_positional_args.size())
					*optional_positional_args[current_optional_arg++].string_value = a;
				else if(remaining_args.type == OPositional)
					*remaining_args.list_value << a;
				else {
		...
		case ExpectingState:
			if(t == Arg) {
				if(currOpt.type == OArg1) {
					*currOpt.string_value = a;
					state = StartState;
				} else if(currOpt.type == ORepeat) {
					currOpt.list_value->append(a);
					state = StartState;
				} else {
					abort();
				}
			} else {
				QString n = currType == LongOpt ? currOpt.lname : QString(
						QChar(currOpt.sname));
			...
			if(t == Arg) {
				if(currOpt.type == OOpt) {
					*currOpt.string_value = a;
					...
					currOpt.list_value->append(origA);
				...
				if(currOpt.type == OOpt)
					*currOpt.string_value = currOpt.def;
				if(t != End) {
...
/**
 ...
 the value is not specified by the user it will be set to \a def.
 */
void GetOpt::addOptionalOption(char s, const QString &l, QString *v,
		const QString &def) {
	Option opt(OOpt, s, l);
	opt.string_value = v;
	opt.def = def;
...
/**
 Registers a required positional command line argument \a name. If the argument
 ...
 isSet() function.
 */
void GetOpt::addPositionalArgument(const QString &name, QString *v) {
	Option opt(OPositional, 0, name);
	opt.string_value = v;
	required_positional_args.push_back(opt);
	*v = QString();
}

/**
 Registers an optional positional command line argument \a name. For a more
 detailed description see the addArgument() documentation.

 */
void GetOpt::addOptionalPositionalArgument(const QString &name, QString *v) {
	Option opt(OPositional, 0, name);
	opt.string_value = v;
	optional_positional_args.push_back(opt);
	*v = QString();
}

/**
 Registers for all remaining positional command line argument \a name. For a more
 detailed description see the addArgument() documentation.
 */
void GetOpt::addRemainingPositionalArguments(const QString &name, QStringList *v) {
	Option opt(OPositional, 0, name);
	opt.list_value = v;
	remaining_args = opt;
	*v = QStringList();
