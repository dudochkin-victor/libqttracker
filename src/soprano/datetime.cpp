...
#include "../include/soprano/datetime.h"
...
static QString utcOffsetToString(int utcOffset)
{
    if (utcOffset == 0)
        return "Z";
    QString ret;
    ret.sprintf("%+03d:%02d", utcOffset / 3600, (utcOffset % 3600) / 60);
    return ret;
}
...
QString Soprano::DateTime::toString( const QDateTime& dt )
{
    ...
    if (dt.timeSpec() == Qt::LocalTime) {
        QDateTime sameInUtc = QDateTime(dt.date(), dt.time(), Qt::UTC);
        utcOffset = dt.secsTo(sameInUtc);
    }
    return toString( dt.date() ) + 'T' + toString( dt.time() ) + utcOffsetToString( utcOffset );
}
