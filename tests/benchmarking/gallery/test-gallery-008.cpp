...
static SopranoLive::RDFSelect allNFOVisualsOnDevice ()
{
...
void GalleryBenchmark::test_gallery_008_benchmark()
{
    QBENCHMARK {
        LiveNodes query_model = ::tracker()->modelQuery (allNFOVisualsOnDevice(),
                                                       SopranoLive::RDFStrategy::LiveStrategy);
        QCOMPARE(query_model->rowCount(), 500);
    }
}

void GalleryBenchmark::test_gallery_008_streaming_1()
{
    QBENCHMARK {
        modelUpdates = 0;
        LiveNodes streaming_model = setStreamingQuery(allNFOVisualsOnDevice());
...
void GalleryBenchmark::test_gallery_008_streaming_end()
{
    QBENCHMARK {
        modelUpdates = 0;
        LiveNodes streaming_model = setStreamingQuery(allNFOVisualsOnDevice());
